...
# source("Clinical.R") # Run to gain the clinical dataframe that's in Output (Clin_UCEC)
# load("FPKMs.RData")

# Looking for the files
thousand.folders <- list.dirs(path = "./UCEC/Data/FPKM", full.names = T)
...
Clin_UCEC <- pap_clin[, c("TCGA.ID", "Microsatellite.instability") ]
Clin_UCEC$Patient.ID <- samptopat(pap_clin$TCGA.ID)
Clin_UCEC$Patient.ID <- gsub("-", ".", Clin_UCEC$Patient.ID)
Clin_UCEC <- Clin_UCEC[!is.na(Clin_UCEC$Microsatellite.instability),]

Clin_UCEC$MSI_STATUS <- ifelse((Clin_UCEC$Microsatellite.instability == "MSI-H"), "MSI-H", "MSS")


CIRC_clin <- merge(Enrichment_CIRC1, Clin_UCEC, by = "Patient.ID")



# pdf("./UCEC/Figures/Clustering/Violin Plot of Mean CIRC.pdf", height = 6, width = 6)
...
pca1 <- CIRC_for_Cluster %>% 
  ...
  merge(Clin_UCEC, by = "Patient.ID") # Merge with cleaned clinical
...
library(factoextra)
# pdf("./UCEC/Figures/Clustering/Number_kmean_cluster.pdf", width = 6, height = 6)
...
# pdf("./UCEC/Figures/Clustering/PhenoG_CIRC.pdf", height = 6, width = 6)
...
# pdf("./UCEC/Figures/Clustering/CIRC_PhenoGraph_tsNE.pdf")
...
# pdf("./UCEC/Figures/Clustering/CIRC_Score_tsNE.pdf")
...
# Phenograph
# pdf("./UCEC/Figures/Clustering/CIRC_Pheno.pdf", height = 6, width = 6)
...
lt <- merge(lessthan, Clin_UCEC, by = "Patient.ID")
...
# pdf("./UCEC/Figures/Gene_Sets/ROS_response.pdf")
...
for(i in levels(Enrich1$Parameter)){
  ...
  temp_plot <- ggplot(work, aes(y = Enrichment, x = CIRC_Genes))+
    ...
    stat_cor()
  filen <- paste0(i, ".pdf")
  ggplot2:: ggsave(filen, plot = temp_plot, device = "pdf", path = "./UCEC/Figures/Gene_Sets/Pearson",
                   height = 6, width = 6)
}
...
for(i in levels(Enrichments$Geneset)){
  ...
  temp_plot <- ggplot(work, aes(y = Enrichment, x = CIRC_Genes))+
    ...
    stat_cor()
  filen <- paste0(i, ".pdf")
  ggplot2:: ggsave(filen, plot = temp_plot, device = "pdf", path = "./UCEC/Figures/",
                   height = 6, width = 6)
}
...
for(i in levels(GOI1$SYMBOL)){
  ...
  filen <- paste0(i, ".pdf")
  ggplot2:: ggsave(filen, plot = temp_plot, device = "pdf",
                   path = "./UCEC/Figures/Genes_of_interest/Correlations",
                   height = 6, width = 6)
}
...
for(i in 1:length(genes_of_interest)){
  ...
  ggplot2:: ggsave(filen, plot = temp_plot, device = "pdf",
                   path = "./UCEC/Figures/Genes_of_interest",
                   height = 6, width = 6)}
...
UCECs <- droplevels(subset(pap_clin, Cancer.type == "UCEC"))

UCECs$Patient.ID <- samptopat(UCECs$TCGA.ID)
UCECs$Patient.ID <- gsub("-", ".", UCECs$Patient.ID)


invest <- merge(UCECs, pat_sub, by = "Patient.ID")
